...
function start() {
  ...
  const main = function () {
    let src = (0, _fs.readFileSync)('src.txt', 'utf8');
  ...
  const requestChatMessages = function (nextPageToken, liveChatId) {
    ...
    request({
      ...
      let key = (0, _fs.readFileSync)('key.txt', 'utf8');
      ...
      if (body.items) {
        ...
        for (body.items.item of body.items) {
          ...
          participants.appendFileSync('./participants.txt', body.items.item.authorDetails.displayName + ' : ' + body.items.item.snippet.displayMessage + '\n'); // console.log(
  ...
  const getLiveChatId = function (videoId, callback) {
    ...
    request(url, function (error, response, body) {
      const bodyObj = JSON.parse(body); //console.log(bodyObj);
  ...
  client.on('chat', function (channel, user, message, self) {
    ...
    participants.appendFileSync('./participants.txt', user['display-name'] + ' : ' + message + '\n');
  });
}
